...
class MedSolrCollections
  ...
  def current_runner
    find { |c| c.might_still_be_running? }
  end
  ...
  def force_release_candidates
    reject { |c| c.aliased? or c.count == 0 }
  end
  ...
  def set_keepers!
    ...
    each { |c| c.allow_deletion! }

    preview&.forbid_deletion!
    production&.forbid_deletion!
    current_runner&.forbid_deletion!

    most_recent_non_failure = self.select { |x| !x.do_not_delete }.reject(&:failure?).first
    most_recent_non_failure&.forbid_deletion!
  end
end
